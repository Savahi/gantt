	...
	function calendar( container, callBack, cellWidth, cellHeight, year, month, day, hour, minute ) {
		if( _calendar !== null ) {
		...
		calendarSetDate( parseInt(year), parseInt(month)-1, parseInt(day), parseInt(hour), parseInt(minute) );
	}
	...
	function calendarInit( container, callBack, cellWidth, cellHeight ) {
		...
		monthInput.style.top = '0px';
		monthInput.style.left = '0px';
		monthInput.style.width = cellWidth*4 + 'px';
		monthInput.style.height = cellHeight + 'px';
		monthInput.style.fontSize = parseInt(3*cellHeight/5);
		...
		for( let w = 0 ; w < 6 ; w++ ) {
			for( let d = 0 ; d < 7 ; d++ ) {
				...
				day.style.top = (w+1)*cellHeight + 'px';
				day.style.left = d*cellWidth + 'px';
				day.style.width = cellWidth + 'px';
				day.style.height = cellHeight + 'px';
				day.style.border = '1px dotted #dfdfdf';
				day.style.textAlign = 'center';
				day.style.fontSize = parseInt(2*cellHeight/3);
				...
				day.onclick = function(e) {
					...
					let d = (_calendarDayChosen < 10) ? '0' + _calendarDayChosen : _calendarDayChosen;
					let hr = document.getElementById('calendar-hour').value;
					let mn = document.getElementById('calendar-minute').value;
					_calendarContainer.removeChild(_calendar);
					_calendar = null;
					container.style.display = 'none';
					_calendarCallBack( {'y':y, 'm':m, 'd':d, 'hr':hr, 'mn':mn } );
				}
		...
		cancel.style.top = cellHeight*7 + 1 + 'px';
		...
		hourInput.style.top = cellHeight*7 + 1 + 'px';
		hourInput.style.left = cellWidth*4 + 'px';
		hourInput.style.width = cellWidth*1 + 'px';
		hourInput.style.height = cellHeight + 'px';
		hourInput.style.fontSize = parseInt(4*cellHeight/6);
		for( let h = 0 ; h < 24 ; h++ ) {
		    let option = document.createElement("option");
		    option.value = h;
		...
		hourMinuteSeparator.style.top = cellHeight*7 + 1 + 'px';
		hourMinuteSeparator.style.left = cellWidth*5 + 'px';
		...
		hourMinuteSeparator.style.fontSize = parseInt(4*cellHeight/5);
		...
		minuteInput.style.top = cellHeight*7 + 1 + 'px';
		minuteInput.style.left = cellWidth*6 + 'px';
		minuteInput.style.width = cellWidth*1 + 'px';
		minuteInput.style.height = cellHeight + 'px';
		minuteInput.style.fontSize = parseInt(4*cellHeight/6);
		for( let m = 0 ; m < 60 ; m++ ) {
		    let option = document.createElement("option");
		    option.value = m;
