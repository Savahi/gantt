...
var _settings = {
	ganttOperation0Color:'#2f8f2f', ganttOperation0Opacity:1.0,
	ganttOperation100Color:'#7f7f7f', ganttOperation100Opacity:1.0,
	ganttPhaseColor:'#0f7f07f', ganttPhaseOpacity:1.0, ganttCriticalColor:'#bf2f2f', 
...
window.addEventListener( 'mouseup', function(e) { 
	...
	if( timeSVG.style.cursor != "default" ) {
		timeSVG.style.cursor = "default";
	}
...
window.addEventListener( 'mousemove', function(e) { 
	if( verticalSplitterCaptured ) {
		...
		drawTableScroll();
		drawGanttHScroll();
		_ganttVisibleWidth *= (ganttSVGWidth / oldGanttSVGWidth);
...
function initLayout() {
	...
	// Gantt chart
	ganttSVG.addEventListener( "mousedown", onGanttMouseDown );
	...
	// To scroll the table vdertically - using the same handler as for the gantt chart... 
	addOnMouseWheel( tableContentSVG, onGanttWheel );

	// Time scale
	timeSVG.onmousedown = onGanttMouseDown;
	timeSVG.onmousemove = onGanttCapturedMouseMove;
	timeSVG.addEventListener( "dblclick", onGanttDblClick );
	...
	for( let i = 0 ; i < _data.operations.length ; i++ ) {
		...
		if( init ) { // Initializing...
			...
			if( _data.operations[i].status == 0 ) { // Not started
				...
				console.log(`${i}: ${xLastFin} / ${xRestart}`);
				op100Properties.id = 'ganttOpFinished'+i;
				let op100;
				...
				if( xLastFin < xRestart ) { // Not a phase
					op100Properties.id = 'ganttOpBetweenFinishedAndNotStarted'+i;
					let opBetween;
					opBetween = createRect( xLastFin, rectTop+rectHeight/4.0, xRestart - xLastFin, rectHeight/2.0, op100Properties  ); // Rectangle
			...
			if( _data.operations[i].status == 0 ) { // Not started
				...
				if( xLastFin < xRestart ) {
					...
					setRectCoords( elBetween, xLastFin, rectTop + rectHeight/4.0, xRestart - xLastFin, rectHeight/2.0 );
				}
...
function drawTableContent( init ) {
	...
	if( init ) {
		...
		for( let col = 0, left=0 ; col < _data.table.length ; col++  ) { // Creating splitters
			left += _data.table[col].width;
			let splitter = createRect( left-tableVisibleLeft, 0, 2, operToScreen(_data.operations.length), 
...
function drawGanttHScroll( init ) {
	...
	let sliderSize = (visibleMaxLeft > _data.visibleMin) ? (ganttHScrollSVGWidth*_ganttVisibleWidth/overallWidth) : ganttHScrollSVGWidth;
	if( sliderSize < _settings.scrollSliderSize ) {
		sliderSize = _settings.scrollSliderSize;
	}
	if( init ) {
		...
		if( visibleMaxLeft > _data.visibleMin ) {
			sliderPosition = (_ganttVisibleLeft-_data.visibleMin) * (ganttHScrollSVGWidth-sliderSize) / (visibleMaxLeft-_data.visibleMin);
		}
...
function drawVerticalScroll( init ) {
	...
	let sliderSize = (visibleMaxTop > 0) ? (verticalScrollSVGHeight*_ganttVisibleHeight/overallHeight) : verticalScrollSVGHeight;
	if( sliderSize < _settings.scrollSliderSize ) {
		sliderSize = _settings.scrollSliderSize;
	}
	if( init ) {
		...
		let sliderPosition = 0;
		if( visibleMaxTop > 0 ) {
			sliderPosition = _ganttVisibleTop * (verticalScrollSVGHeight-sliderSize) / visibleMaxTop;
		}
...
function onGanttMouseDown(e) {
	...
	ganttCapturedLeft = _ganttVisibleLeft;			
...
function onGanttCapturedMouseMove(e) {
	...
	let deltaX = _ganttVisibleWidth * (e.clientX - ganttCapturedAtX) / ganttSVGWidth;
	_ganttVisibleLeft = validateGanttLeft( ganttCapturedLeft - deltaX );
	ganttCapturedLeft = _ganttVisibleLeft;
	ganttCapturedAtX = e.clientX;
	drawGantt();
	drawTimeScale();
...
function screenToTime( screenX ) {
	let xNotScaled = _ganttVisibleLeft + screenX * (_ganttVisibleWidth - 1) / (ganttSVGWidth-1);
}


function operToScreen( n ) {
	//return ( ( n ) * ganttSVGHeight )/ _data.operations.length; 
	return ( n - _ganttVisibleTop) * ganttSVGHeight / (_ganttVisibleHeight+0.5); 
} 
