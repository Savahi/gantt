...
var userDataSaveScript = "gantt_user_data.php"
...
window.addEventListener( 'mouseup', function(e) { 
	...
	if( tableSplitterCaptured >= 0 ) {
		...
		let newWidth = data.table[tableSplitterCaptured].width + e.x - tableSplitterCapturedAtX;
		if( newWidth < 4 ) {
			newWidth = 4;
		}
		data.table[tableSplitterCaptured].width = newWidth;
		    ...
		    if (this.readyState == 4 ) {
		    	if( this.status == 200) {
			    	...
			    	if( errorParsingData ) {
				        ...
				        xmlhttpUserData.open("GET", userDataFile, true);
...
function drawGantt( init ) {
	...
	for( let i = 0 ; i < data.links.length ; i++ ) {
		...
		let PredCode = data.links[i].PredCode;
		let SuccCode = data.links[i].SuccCode;
		let predOp = null;
		let succOp = null;
	...
	for( let i = 0 ; i < data.operations.length ; i++ ) {
		if( !data.operations[i].visible ) { // Optimizing.
		...
		for( let i = 0 ; i < data.operations.length ; i++ ) { // For all operations...
			for( let iU = 0 ; iU < userData.length ; iU++ ) { // For all userData items...
				if( data.operations[i].Code == userData[iU].operationCode ) { // If the codes are the same...
					...
					for( let iE=0 ; iE < data.editables.length ; iE++ ) {
...
function createEditBoxInputs() {
	...
	for( let iE = 0 ; iE < data.editables.length ; iE++ ) {
		...
		let input;
		if( data.editables[iE].type == 'text' ) {
...
function drawTableContent( init ) {
	...
	for( let i = 0 ; i < data.operations.length ; i++ ) {
		...
		if( init ) {			
			expandText = createText( expand, data.table[0].width/2.0, lineMiddle, 
			...
			expandText.setAttributeNS(null,'x',data.table[0].width/2.0);
...
function saveUserDataFromEditBox() {
	...
	if( document.location.host ) {
		...
		if( userData.length > 0 ) {
			xmlhttp.open("POST", userDataSaveScript, true);
			xmlhttp.setRequestHeader("Cache-Control", "no-cache");
